...
def do(action, state):
    ...
    n_foxes = cards.count('F')
    n_hens = cards.count('H')
    next_card = random.choice(cards)
    if next_card == 'H':
        new_card = 'F' * n_foxes + 'H' * (n_hens - 1)
        new_yard = yard + 1
        new_score = score
    if next_card == 'F':
        new_card = 'F' * (n_foxes - 1) + 'H' * n_hens
        new_yard = 0
        new_score = score
    if action == 'gather':
        new_yard = 0
        new_score = score + yard
    return new_score, new_yard, new_card
...
def superior(A, B=take5):
    ...
    print '%d, %d, %d' % (avg_A, avg_B, avg_A - avg_B)
    return avg_A - avg_B > 1.5

def strategy(state):
    (score, yard, cards) = state
    if 'F' not in cards or yard < 3:
        return 'wait'
    return 'gather'
