...
def more_pour_problem(capacities, goal, start=None):
    ...
    def get_successors(state):
        ...
        for i, x in enumerate(state):
            ...
            # Use replace() to get a new tuple
            # replace(state, i, capacities[i])
            ...
            if x == 0:
                ...
                ans[tuple(next_state)] = ('fill', i)
            else:
                next_state[i] = 0
                ans[tuple(next_state)] = ('empty', i)
                for j, y in enumerate(state):
                    if (j != i) and (y < capacities[j]):
                        next_state = list(state)
                        delta = min(capacities[j] - y, state[i])
                        next_state[i] -= delta
                        next_state[j] += delta
                        ans[tuple(next_state)] = ('pour', i, j)
        return ans

    return shortest_path_search(start, get_successors, is_goal)

        # return dict((('fill', index) for index, x in enumerate(state) if x == 0) +
        #             (('empty', index) for index, x in enumerate(state) if x > 0) +
        #             (('pour', i, j) for i, x in enumerate))
