...
def best_hand(hand):
    ...
    # We need combinations here
    # not permutations
    # cause we need hand to be sorted

    # My solution
    result, maxval = None, None
    for five_card in itertools.permutations(hand, 5):
        val = hand_rank(list(five_card))
        if not result or val > maxval:
            result, maxval = five_card, val
    return result
